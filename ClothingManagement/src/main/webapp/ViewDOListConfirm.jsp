...
<form id="poForm" action="DeliveryOrderConfirmServlet" method="get">
...
<table border="1">
    ...
    <c:forEach var="dos" items="${deliveryOrders}">
        <tr>
            ...
            <td>${dos.receiptDate == '1970-01-01' ? 'Not receipt' : dos.receiptDate}</td>
            <td>${dos.poID}</td>
            <td>${createdByList[dos.createdBy] != null ? createdByList[dos.createdBy] : "Unknown CreatedBy"}</td>
            <td>${recipientList[dos.recipient] != null ? recipientList[dos.recipient] : "Unknown Recipient"}</td>
            <td>${dos.status ? 'Hoàn thành' : 'Chưa hoàn thành'}</td>
            <td>
                ...
                <!-- Chỉ hiển thị nút Confirm nếu status == true -->
                <c:if test="${!dos.status}">
                    <form action="ConfirmDOServlet" method="get" style="display:inline;">
                        <input type="hidden" name="doId" value="${dos.doID}">
                        <input type="hidden" name="createdBy"  value="<%=session.getAttribute("account_id") %>" readonly>
